...
function Dnd({ reservations, tomorrowNight }) {
    ...
    const [pitchReservations, setPitchReservations] = useState(
        Object.entries(reservations).map(([pitchName, reservations]) => ({
            pitchName,
            reservations: reservations.filter(reservation => reservation.visible === true)
        }))
    );
    ...
    useEffect(() => {
        if (tomorrowNight) setTomorrowNightReservations(
    ...
    const determineSourcePitch = (source) => {
        if (source.index < pitchReservations[0].reservations.length) {
            return pitchReservations.find(pitch => pitch.pitchName === source.droppableId);
        } else {
            return tomorrowNightReservations.find(pitch => pitch.pitchName === source.droppableId);
        }
    }

    const determineDestinationPitch = (destination) => {
        if (destination.index < pitchReservations[0].reservations.length) {
            return pitchReservations.find(pitch => pitch.pitchName === destination.droppableId);
        } else {
            return tomorrowNightReservations.find(pitch => pitch.pitchName === destination.droppableId);
        }
    }
    ...
    const reservationsOnDragEnd = (source, destination) => {
        ...
        updateDatabaseOnDragEnd(source.droppableId, destination.droppableId, sourceItems[source.index].hour, destItems[destination.index].hour, sourceItems[source.index].date, destItems[destination.index].date);
    }
    ...
    const crossReservationsOnDragEnd = (source, destination) => {
        ...
        updateDatabaseOnDragEnd(source.droppableId, destination.droppableId, sourceItems[sourceIndex].hour, destItems[destinationIndex].hour, sourceItems[sourceIndex].date, destItems[destinationIndex].date);
    }
    ...
    const updateSourcePitchReservations = (source, sourceItems) => {
        ...
        if (sourcePitch.reservations.length > tomorrowNightReservations[0].reservations.length) {

            setPitchReservations(prevReservations => prevReservations.map(pitch => {
                if (pitch.pitchName === source.droppableId) {
                    return { ...pitch, reservations: sourceItems };
                }
                return pitch;
            ...
            setTomorrowNightReservations(prevReservations => prevReservations.map(pitch => {
                if (pitch.pitchName === source.droppableId) {
                    return { ...pitch, reservations: sourceItems };
                }
                return pitch;
    ...
    const updateDestinationPitchReservations = (destination, destItems) => {
        ...
        if (destinationPitch.reservations.length > tomorrowNightReservations[0].reservations.length) {
                                ...
                                <ul className="space-y-2" {...provided.droppableProps} ref={provided.innerRef}>
                                    {pitch.reservations.map((item, index) => item.visible && (
                                        <li className="bg-slate-700 rounded shadow-md h-20" key={item.hour}>
                                            <Draggable key={'today-' + pitch.pitchName + '-' + item.hour} draggableId={'today-' + pitch.pitchName + '-' + item.hour} index={index}>
                                                {(provided, snapshot) => (
                                                    <a
                                                        onClick={() => handleReservationClick(pitch.pitchName, item)}
                                                        ref={provided.innerRef}
                                                        {...provided.draggableProps}
                                                        {...provided.dragHandleProps}
                                                    >
                                                        <div className={`flex flex-col text-center md:w-52 py-2 px-2 ${handleWidth()}`}>
                                                            <div className='flex flex-row justify-between align-baseline items-baseline'>
                                                                <p className={`mr-2 text-lg text-left font-bold ${snapshot.isDragging ? 'text-transparent' : ''}`}>{item.hour + ':' + item.minute}</p>
                                                                <p className="text-sm flex-1 font-semibold truncate">{showReservedOrNot(item)}</p>
                                                            </div>
                                                            <p className="text-lg font-semibold h-10 truncate pt-1">{user.type === 'admin' ? item.note : ''}</p>
                                                        </div>
                                                    </a>
                                                )}
                                            </Draggable>
                                    ...
                                    {tomorrowNight && tomorrowNightReservations[index]?.reservations.map((item, index) => item.visible && (
                                        <li className="bg-slate-700 rounded shadow-md h-20" key={item.hour}>
                                            <Draggable key={'tomorrow-' + pitch.pitchName + '-' + item.hour} draggableId={'tomorrow-' + pitch.pitchName + '-' + item.hour} index={index + pitchReservations[0]?.reservations?.length}>
                                                {(provided, snapshot) => (
                                                    <a
                                                        onClick={() => handleReservationClick(pitch.pitchName, item)}
                                                        ref={provided.innerRef}
